...
<h3>6.4 Profit, and What a Request Actually Costs</h3>
<p>After engineering, sales and admin, the company loses $2.37 million in 2026 and $3.21 million in 2027, then makes $11.23 million in 2028. The deepest point is $5.58 million of cumulative losses, in late 2027. The assumption most likely to be wrong is the number of practices, not the price. If all four of our main assumptions are wrong at once, 2028 revenue is $13.8 million rather than $36.1 million: a smaller company, but still a real one.</p>

<p>Two per-request costs appear in this report and they measure different things, so it is worth saying which is which. Dividing the cloud infrastructure line in Table 8 by the requests processed gives 31 cents in 2026, 12 cents in 2027 and 10 cents in 2028. That line covers compute, the database, object storage, egress and seven years of audit retention, so it charges every request for records that must be kept long after the request has been answered. The marginal cost of one more authorization is the compute alone: fixed rules over a single database read, no model call, a few milliseconds of processor time. That is the six hundredths of a cent quoted in section 6.2, and it is the figure that matters when volume doubles.</p>

<p>The model API line behaves differently again, falling from 6 cents per request to 3 across the three years. None of it is spent on the authorization path: those tokens read insurance documents and draft appeals. That is why Table 5 can say an AI outage leaves authorization unaffected, and why the cost of deciding does not move when model prices do.</p>
