...
3. **The speed claim must be stated carefully.** The *agents* reach a decision in 88.6
   ...
   carry the distinction. Never imply a patient gets an answer in 88 milliseconds.
...
### Measured system performance (measured 13 Sept 2026 against committed code)
| Metric | Value |
|---|---|
| Complete authorization decision, start to finish | 88.6 milliseconds |
| Two agents agreeing, network excluded | 44.7 milliseconds |
| Creating one zero-knowledge privacy proof | 406 milliseconds |
| Full six-step guided demonstration | 11.1 seconds |
| Automated tests passing | 138 of 138 |

### Build metrics (repository `hwangus922/AI2O-Pavo-Cloud`, through commit `a13f6cb`)
| Metric | Value |
|---|---|
| Lines of code | 19,541 insertions, 367 deletions |
| File changes | 170 |
| Commits | 12, in 5 reviewed batches |
| Build window | 7–12 September 2026 (6 days) |
...
| 2 | Clear cases close in milliseconds | The two agents agree in 44.7ms; the full decision path takes 88.6ms. |
...
**Footer strip** "Typical time from step 1 to step 7: **88.6 milliseconds**, measured over
...
| 138 | automated tests, all passing |
| 19,541 | lines of code, 170 files |
| 12 | commits in 5 reviewed batches |
| 88.6 ms | end-to-end decision |
| 6 days | from first commit to working system |
**Right panel** — the commit history in a mono terminal block (grey `#F4F4F4` fill, 1px
`#CCCCCC` border, 16px mono), using the six commits verbatim from section 3. Caption under
it in 16px italic: "Reviewed and merged in five batches, 7–12 September 2026."
...
or **— (grey dash)**. Header row in blue bold, 1px `#555555` under it; 1px `#DCDCDC`
...
### Slide 11 — Market
**Visual** Three concentric arcs or three nested bars, left to right, largest to smallest —
**not** a pie. Label each with figure, name and one qualifier.
...
**Visual — burn curve, not a pie.** A small line chart: cumulative EBITDA across 2026–28,
...
**Three priority blocks below** (what the money de-risks, in order — describe as priorities,
